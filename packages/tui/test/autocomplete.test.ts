...
describe("CombinedAutocompleteProvider", () => {
	...
	describe("slash command separators", () => {
		it("uses CJK punctuation between a command name and its argument completion", async () => {
			const provider = new CombinedAutocompleteProvider(
				[
					{
						name: "search",
						getArgumentCompletions: (prefix: string) => [{ value: `result:${prefix}`, label: prefix }],
					},
				],
				"/tmp",
			);
			for (const separator of [" ", "，", "。"]) {
				const line = `/search${separator}文档`;
				const result = await getSuggestions(provider, [line], 0, line.length);
				assert.ok(result, line);
				assert.strictEqual(result.prefix, "文档");
				assert.deepStrictEqual(result.items.map((item) => item.value), ["result:文档"]);
			}
		});

		it("allows Tab file completion after a slash command boundary", () => {
			const provider = new CombinedAutocompleteProvider([], "/tmp");
			assert.strictEqual(provider.shouldTriggerFileCompletion(["/search"], 0, 7), false);
			for (const separator of ["，", "。"]) {
				const line = `/search${separator}文档`;
				assert.strictEqual(provider.shouldTriggerFileCompletion([line], 0, line.length), true, line);
			}
		});
	});
	...
	describe("dot-slash path completion", () => {
		...
		test("does not show a directory listing for natural completion at an empty token", async () => {
			setupFolder(baseDir, { files: { "说明.md": "text" } });
			const provider = new CombinedAutocompleteProvider([], baseDir);
			for (const separator of [" ", "\t", "\u3000", "，", "。"]) {
				const line = `查看${separator}`;
				assert.strictEqual(await getSuggestions(provider, [line], 0, line.length, false), null);
				const forced = await getSuggestions(provider, [line], 0, line.length, true);
				assert.ok(forced, line);
				assert.strictEqual(forced.prefix, "");
				assert.deepStrictEqual(
