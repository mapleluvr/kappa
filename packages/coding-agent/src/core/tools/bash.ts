...
export const bashToolSystemPromptContribution = {
	...
	guidelines: ["You can inspect KAPPA_* environment variables for current model and session details."],
} as const;
...
function resolveSpawnContext(
	...
	if (exposeSessionEnvironment && ctx) {
		...
		env.KAPPA_SESSION_ID = ctx.sessionManager.getSessionId();
		const sessionFile = ctx.sessionManager.getSessionFile();
		if (sessionFile) env.KAPPA_SESSION_FILE = sessionFile;
		if (model) {
			env.KAPPA_PROVIDER = model.provider;
			env.KAPPA_MODEL = model.id;
		}
		if (ctx.thinkingLevel) env.KAPPA_REASONING_LEVEL = ctx.thinkingLevel;
	}
...
export interface BashToolOptions {
	...
	shellPath?: string;
	/** Expose current Kappa session metadata as KAPPA_* environment variables. Default: true */
	exposeSessionEnvironment?: boolean;
