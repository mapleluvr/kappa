...
describe("AgentSession dynamic tool registration", () => {
	...
	it("exposes session state before custom bash spawn hooks and supports opting out", async () => {
		...
		expect(session.systemPrompt).toContain(
			"You can inspect KAPPA_* environment variables for current model and session details.",
		);
		await bashTool.execute("bash-env", { command: "printf ok" });
		expect(sessionEnv).toMatchObject({
			KAPPA_SESSION_ID: session.sessionId,
			KAPPA_SESSION_FILE: session.sessionFile,
			KAPPA_PROVIDER: model.provider,
			KAPPA_MODEL: model.id,
			KAPPA_REASONING_LEVEL: session.thinkingLevel,
		});
		...
		expect(optedOutEnv).not.toHaveProperty("KAPPA_SESSION_ID");
		expect(optedOutEnv).not.toHaveProperty("KAPPA_SESSION_FILE");
		expect(optedOutEnv).not.toHaveProperty("KAPPA_PROVIDER");
		expect(optedOutEnv).not.toHaveProperty("KAPPA_MODEL");
		expect(optedOutEnv).not.toHaveProperty("KAPPA_REASONING_LEVEL");
