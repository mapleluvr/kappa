...
const originalKappaPackageDir = process.env.KAPPA_PACKAGE_DIR;
...
afterEach(() => {
	...
	if (originalKappaPackageDir === undefined) {
		delete process.env.KAPPA_PACKAGE_DIR;
	} else {
		process.env.KAPPA_PACKAGE_DIR = originalKappaPackageDir;
	}
...
function createNpmPrefixInstall(template = "pi-prefix-"): { prefix: string; packageDir: string } {
	...
	tempDir = prefix;
	process.env.KAPPA_PACKAGE_DIR = packageDir;
...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		...
		expect(getUpdateInstruction("@mapleluvr/kappa-coding-agent")).toBe(
			"Run: pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @mapleluvr/kappa-coding-agent",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@mapleluvr/kappa-coding-agent")).toBeUndefined();
		expect(getUpdateInstruction("@mapleluvr/kappa-coding-agent")).toBe(
			"Update @mapleluvr/kappa-coding-agent using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("@mapleluvr/kappa-coding-agent");
		...
		expect(command).toEqual({
			...
			args: [
				...
				"--ignore-scripts",
				"--min-release-age=0",
				"@mapleluvr/kappa-coding-agent",
			],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @mapleluvr/kappa-coding-agent`,
		});
	});
	...
	test("self-updates exact npm versions without uninstalling the current package", () => {
		...
		const command = getSelfUpdateCommand("@mapleluvr/kappa-coding-agent", undefined, {
			packageName: "@mapleluvr/kappa-coding-agent",
			installSpec: "@mapleluvr/kappa-coding-agent@1.2.3",
		});
		...
		expect(command).toEqual({
			...
			args: [
				...
				"--ignore-scripts",
				"--min-release-age=0",
				"@mapleluvr/kappa-coding-agent@1.2.3",
			],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @mapleluvr/kappa-coding-agent@1.2.3`,
		});
	});
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@mapleluvr/kappa-coding-agent", ["npm", "--prefix", prefix]);
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(
			`npm --prefix "${prefix}" install -g --ignore-scripts --min-release-age=0 @mapleluvr/kappa-coding-agent`,
		);
	});
	...
	test("does not infer Windows npm custom prefixes from package paths", () => {
		...
		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@mapleluvr/kappa-coding-agent")).toBe(
			"Run: npm install -g --ignore-scripts --min-release-age=0 @mapleluvr/kappa-coding-agent",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@mapleluvr/kappa-coding-agent"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 @mapleluvr/kappa-coding-agent",
		});
	});
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		const packageName = "@mapleluvr/kappa-coding-agent";
		...
		tempDir = temp;
		process.env.PATH = `${binDir}${delimiter}${originalPath ?? ""}`;
		process.env.KAPPA_PACKAGE_DIR = storePackageDir;
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@mapleluvr/kappa-coding-agent")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@mapleluvr/kappa-coding-agent")).toContain(
