...
const packages = [
	...
	{ directory: "packages/tui", name: "@mapleluvr/kappa-tui" },
	...
	{ directory: "packages/coding-agent", name: "@mapleluvr/kappa-coding-agent" },
];
...
function createKappaShim(installDirectory) {
	const binDirectory = join(installDirectory, "node_modules", ".bin");
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "kappa.cmd"))) {
			writeFileSync(join(installDirectory, "kappa.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\kappa.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "kappa.ps1"), '& "$PSScriptRoot/node_modules/.bin/kappa.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "kappa.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\kappa.exe" %*\r\n');
		writeFileSync(join(installDirectory, "kappa.ps1"), '& "$PSScriptRoot/node_modules/.bin/kappa.exe" @args\n');
		return;
	}
	symlinkSync(join("node_modules", ".bin", "kappa"), join(installDirectory, "kappa"));
}
...
if (!options.skipInstall) {
	...
	createKappaShim(nodeInstallDirectory);
	...
	if (!options.skipBunInstall) {
		...
		createKappaShim(bunInstallDirectory);
	}
}
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "kappa.cmd" : "kappa")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "kappa.cmd" : "kappa")} --help`);
	}
}
